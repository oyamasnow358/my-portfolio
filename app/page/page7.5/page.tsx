...
export default function LessonLibraryPage() {
  ...
  const loadGoogleSheet = async () => {
    ...
    try {
      // 公開CSVからデータを取得
      // ※注意: GOOGLE_SHEET_CSV_URL がダミーだとエラーになります
  ...
  const generateExcel = async () => {
    ...
    try {
      ...
      Object.keys(mapping).forEach(key => {
        ...
        let val = rowData[key] || "";
        // リスト形式（セミコロン区切り）への対応などが必要ならここで処理
            ...
            >
              ...
              {totalPages > 1 && (
                <div className="flex justify-center gap-2">
                  <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-4 py-2 rounded-lg bg-white border disabled:opacity-50">Prev</button>
                  <span className="px-4 py-2 font-bold text-gray-600">{currentPage} / {totalPages}</span>
                  <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-4 py-2 rounded-lg bg-white border disabled:opacity-50">Next</button>
                </div>
              )}
            </motion.div>
          )}
...
// 詳細ページコンポーネント (コードが長くなるため、先ほどのDetailPageと同じものを使用)
function DetailPage({ lesson, onBack, showFlow, setShowFlow, allLessons, onSelectLesson }: any) {
  // ... (先ほどのDetailPageの実装をそのまま貼り付け)
  // スペース省略のため、ここに先ほどのDetailPageコンポーネントの中身を入れてください。
  // もし分からなければ、先ほどのコードのDetailPage部分をここにコピペしてください。
  return (
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-8 md:p-12 shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {lesson.detail_word_url && <a href={lesson.detail_word_url} target="_blank" className="flex items-center justify-center gap-3 p-5 rounded-xl font-bold text-lg bg-blue-600 hover:bg-blue-500 transition-all"><FileText/> 指導案 (Word)</a>}
            {lesson.detail_pdf_url && <a href={lesson.detail_pdf_url} target="_blank" className="flex items-center justify-center gap-3 p-5 rounded-xl font-bold text-lg bg-red-600 hover:bg-red-500 transition-all"><FileText/> 指導案 (PDF)</a>}
          </div>
        </div>
      </div>
    </div>
